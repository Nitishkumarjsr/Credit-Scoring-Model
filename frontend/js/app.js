 ...
 * dataset exploration, batch portfolio underwriting, and Supabase audit history.
 */
...
const state = {
    ...
    batchResults: [],
    historyLimit: 25,
    isRenderWaking: false
};

// ==========================================
// Cloud Health Monitor & Status Badges
// ==========================================
async function initCloudHealthMonitor() {
    updateCloudStatusBadge({ status: "checking" });

    // Initial check
    const health = await window.API_CONFIG.checkHealth();
    updateCloudStatusBadge(health);
...
function updateCloudStatusBadge(health) {
    ...
    if (health.online && health.data) {
        ...
        if (supabaseBadge) {
            if (dbInfo.connected) {
                supabaseBadge.innerHTML = `<span class="pulse-dot"></span><span>Supabase DB: <strong>Connected</strong></span>`;
                supabaseBadge.className = "status-pill pill-success";
            } else if (dbInfo.status === "not_configured") {
                supabaseBadge.innerHTML = `<span class="pulse-dot pulse-amber"></span><span>DB: <strong>Local Memory</strong></span>`;
                supabaseBadge.className = "status-pill pill-neutral";
                supabaseBadge.title = "SUPABASE_URL not configured. Running in memory fallback mode.";
            } else {
                supabaseBadge.innerHTML = `<span class="pulse-dot pulse-amber"></span><span>DB: <strong>${dbInfo.status}</strong></span>`;
                supabaseBadge.className = "status-pill pill-warning";
            }
        }
    } else {
        if (renderBadge) {
...
function getApplicantDataFromForm() {
    return {
        applicant_name: "Applicant-" + Math.floor(1000 + Math.random() * 9000),
    ...
    presets.forEach(p => {
        ...
        if (btn) {
            btn.addEventListener("click", async () => {
                ...
                try {
                    const endpoint = window.API_CONFIG.endpoint(`/api/sample/${p}`);
...
async function runPrediction() {
    ...
    try {
        const endpoint = window.API_CONFIG.endpoint("/api/predict");
        const res = await fetch(endpoint, {
            method: "POST",
...
function renderPredictionResults(result) {
    ...
    const persistBadge = document.getElementById("persisted-badge");
    ...
    if (persistBadge) {
        if (persisted_to === "supabase") {
            persistBadge.innerHTML = "⚡ Synced to Supabase";
            persistBadge.className = "persisted-tag tag-supabase";
        } else {
            persistBadge.innerHTML = "💾 Logged in Memory";
            persistBadge.className = "persisted-tag tag-memory";
        }
    }
...
async function loadBenchmarks() {
    try {
        const endpoint = window.API_CONFIG.endpoint("/api/metrics");
        const res = await fetch(endpoint);
        const data = await res.json();
        if (data.status === "success") {
            ...
            if (gb) {
                ...
                if (kpiAcc) kpiAcc.textContent = `${(gb.accuracy * 100).toFixed(2)}%`;
                if (kpiAuc) kpiAuc.textContent = gb.roc_auc.toFixed(4);
            }
        }
    } catch (e) {
...
async function loadDatasetRecords(offset = 0) {
    try {
        const endpoint = window.API_CONFIG.endpoint(`/api/dataset/records?limit=${state.datasetLimit}&offset=${offset}&filter=${encodeURIComponent(state.datasetFilter)}`);
        const res = await fetch(endpoint);
        const data = await res.json();
...
// ==========================================
// Underwriting Audit History & Supabase Tab
// ==========================================
function initHistoryTab() {
    ...
    if (refreshBtn) {
        refreshBtn.addEventListener("click", () => {
            ...
            loadAuditHistory().finally(() => refreshBtn.classList.remove("loading"));
        });
    }
}
...
async function loadAuditHistory() {
    ...
    try {
        const endpoint = window.API_CONFIG.endpoint(`/api/history?limit=${state.historyLimit}`);
        const res = await fetch(endpoint);
        const data = await res.json();
        ...
        if (data.status === "success" && tbody) {
            ...
            if (storageBadge) {
                storageBadge.textContent = data.storage === "supabase" ? "⚡ Live Supabase PostgreSQL" : "💾 In-Memory Cache";
                storageBadge.className = data.storage === "supabase" ? "subcard-badge badge-approved" : "subcard-badge badge-neutral";
            }

            if (records.length === 0) {
                tbody.innerHTML = '<tr><td colspan="8" style="text-align:center; padding: 2rem; color: #64748b;">No underwriting records yet. Run a prediction in the Loan Simulator to create an audit log entry!</td></tr>';
                return;
            }
            ...
            records.forEach((r, idx) => {
                ...
                tr.innerHTML = `
                    ...
                    <td>${timeStr}</td>
                    <td><strong>${r.fico_score || '-'}</strong></td>
                    <td>${statusBadge}</td>
                    <td>${r.risk_tier || '-'}</td>
                    <td>$${Number(r.loan_amnt || 0).toLocaleString()}</td>
                    <td>$${Number(r.person_income || 0).toLocaleString()}</td>
                    <td><small style="color:#94a3b8;">${r.model_agreement || '3/3 Models'}</small></td>
                `;
                tbody.appendChild(tr);
            });
        }
...
function initBatchProcessing() {
    ...
    if (sampleBatchBtn) {
        sampleBatchBtn.addEventListener("click", async () => {
            ...
            try {
                const endpointRecs = window.API_CONFIG.endpoint("/api/dataset/records?limit=20&offset=0");
                const res = await fetch(endpointRecs);
                const data = await res.json();
                if (data.status === "success") {
                    const applicants = data.data.records.map(r => r.data);
                    const endpointBatch = window.API_CONFIG.endpoint("/api/batch-predict");
                    const batchRes = await fetch(endpointBatch, {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
...
async function processBatchFile(file) {
    ...
    try {
        const endpoint = window.API_CONFIG.endpoint("/api/batch-predict");
        const res = await fetch(endpoint, {
            method: "POST",
            body: formData
        });
